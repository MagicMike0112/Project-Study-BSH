...
async function handleOvenStatusList(req, res, userId) {
  if (req.method !== "GET") return res.status(405).end();
  const haId = req.query?.haId ? String(req.query.haId) : "";
  if (!haId) return res.status(400).json({ ok: false, error: "Missing query param: haId" });
  ...
  const data = await hcFetchJson(
    userId,
    `/api/homeappliances/${encodeURIComponent(haId)}/status`,
    { method: "GET" },
  );
  return res.status(200).json({ ok: true, haId, raw: data });
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'ovenStatusList': return await handleOvenStatusList(req, res, userId);
